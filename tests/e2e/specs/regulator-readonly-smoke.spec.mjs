...
test('regulator lands on /risk and only sees read-only regulator menu entries', async ({ page, request }) => {
  ...
  await expect(sidebar).toContainText('风险查看')
  await expect(sidebar).toContainText('批次查看')
  await expect(sidebar).toContainText('质检查看')
...
test('regulator can read /batches, /batches/2, /quality and /risk without seeing write actions', async ({ page, request }) => {
  ...
  const firstRiskStatus = page.locator('[data-testid="batch-risk-status"]').first()
  await expect(firstBatchCard).toBeVisible()
  await expect(firstBatchCard).toContainText('最近更新')
  await expect(firstRiskStatus).toBeVisible()
  await expect(firstRiskStatus).not.toBeEmpty()
  ...
  await expect(page.getByTestId('workbench-quality-panel')).toBeVisible()
  await expect(page.getByTestId('workbench-qr-panel')).toBeVisible()
  await expect(page.getByTestId('workbench-risk-panel')).toBeVisible()
  await expect(page.getByTestId('workbench-assignment-panel')).toContainText('只查看分配结果')
  await expect(page.getByTestId('workbench-quality-panel')).toContainText('报告编号')
  await expect(page.getByTestId('workbench-latest-record')).toBeVisible()
  ...
  await expect(firstRiskRow).toContainText('最近更新')
