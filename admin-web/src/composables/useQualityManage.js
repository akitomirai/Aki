...
export function useQualityManage() {
    ...
    const canWrite = computed(() => roleCode.value === 'ENTERPRISE_ADMIN' || roleCode.value === 'ENTERPRISE_USER' || roleCode.value === 'ADMIN')
    ...
    async function handleSubmit() {
        ...
        if (form.reportJson && !isValidJson(form.reportJson)) {
            ElMessage.error('扩展信息不是合法 JSON，请检查大括号和引号')
            return
        }
...
function formatJsonInputFactory(targetRefName) {
    return function(target) {
        if (!target[targetRefName]) {
            ElMessage.warning('请先输入JSON内容')
            return
        }
        try {
            target[targetRefName] = formatJsonText(target[targetRefName])
...
const jsonTemplateOptions = [
    { label: '通用检测项模板', value: 'default' },
    { label: '农残检测模板', value: 'pesticide' },
    { label: '微生物检测模板', value: 'microbes' }
]

const formatJsonInput = (targetType, payload) => {
    const formatters = {
        create: formatJsonInputFactory('reportJson'),
        edit: formatJsonInputFactory('reportJson')
    }
    formatters[targetType]?.(payload)
}

const fillJsonExample = (targetType, payload) => {
    payload.reportJson = defaultJsonExample
    ElMessage.success(targetType === 'create' ? '已填充新增示例JSON' : '已填充编辑示例JSON')
}
...
const applyJsonTemplate = (targetType, payload, templateKey) => {
    ...
    payload.reportJson = templateMap[templateKey] || defaultJsonExample
    ElMessage.success(targetType === 'create' ? '已应用模板到新增表单' : '已应用模板到编辑表单')
}

const clearJsonInput = (targetType, payload) => {
    payload.reportJson = ''
    ElMessage.success(targetType === 'create' ? '已清空新增JSON' : '已清空编辑JSON')
}
