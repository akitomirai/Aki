...
$screenshotDirName = -join ([char[]](0x622A, 0x56FE))
$jsonPath = Join-Path (Join-Path (Join-Path (Join-Path $root $miscDirName) $logDirName) 'playwright') 'full-regression-baseline.json'
$scriptPath = Join-Path (Join-Path (Join-Path (Join-Path $root $miscDirName) $logDirName) 'playwright') 'full-regression-baseline.mjs'
$screenshotDir = Join-Path (Join-Path (Join-Path $root $miscDirName) $screenshotDirName) 'playwright'
