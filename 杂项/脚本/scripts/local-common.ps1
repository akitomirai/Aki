...
$script:MiscDirName = -join ([char[]](0x6742, 0x9879))
$script:LogDirName = -join ([char[]](0x65E5, 0x5FD7))
$script:RuntimeDir = Join-Path (Join-Path (Join-Path $script:ProjectRoot $script:MiscDirName) $script:LogDirName) 'runtime'
...
function Test-ProjectProcessRecord {
    ...
    if ($commandLine -like '*edu.jxust.agritrace.TraceabilityBackendApplication*') {
        return $true
    }
    return $false
}
