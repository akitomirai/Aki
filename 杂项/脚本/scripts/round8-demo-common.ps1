...
$script:Round8Root = (Resolve-Path (Join-Path $PSScriptRoot '..\..\..')).Path
$script:Round8MiscDirName = -join ([char[]](0x6742, 0x9879))
$script:Round8LogDirName = -join ([char[]](0x65E5, 0x5FD7))
$script:Round8ScreenshotDirName = -join ([char[]](0x622A, 0x56FE))
$script:Round8ArtifactsDir = Join-Path (Join-Path (Join-Path $script:Round8Root $script:Round8MiscDirName) $script:Round8LogDirName) 'artifacts'
$script:Round8LogsDir = $script:Round8ArtifactsDir
...
$script:Round8ScreenshotsDir = Join-Path (Join-Path (Join-Path (Join-Path $script:Round8Root $script:Round8MiscDirName) $script:Round8ScreenshotDirName) 'artifacts') 'screenshots'
